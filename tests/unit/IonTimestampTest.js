...
define(
  function(require) {
    ...
    parseTest('Parses timestamp: Happy New Year in UTC, unknown local offset', '2007-01-01T00:00-00:00');
      ...
      const constructorTest = function(precision, offset, year, month, day, hour, minute, seconds, decimal, timestamp) {
          suite[name] = function() {
              let time = new ion.Timestamp(params[0], params[1], params[2], params[3], params[4], params[5]);
          }
      };


      const apiContractTest = function(params, timestamp) {
          suite[name] = function() {
              let time = new ion.Timestamp(params[0], params[1], params[2], params[3], params[4], params[5]);
          }
      };
      ...
      const textSerializationTest = function(params, timestamp) {
          suite[name] = function() {
              let time = new ion.Timestamp(params[0], params[1], params[2], params[3], params[4], params[5]);
          }
      };

      const binarySerializationTest = function(params, timestamp) {
          suite[name] = function() {
              let time = new ion.Timestamp(params[0], params[1], params[2], params[3], params[4], params[5]);
          }
      };
      ...
      const binaryDeserializeTest = function(params, expected) {
          suite[name] = function() {
              let time = new ion.Timestamp(params[0], params[1], params[2], params[3], params[4], params[5]);
          }
      };
