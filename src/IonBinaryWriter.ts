...
export class BinaryWriter extends AbstractWriter {
  ...
  writeBlob(value: Uint8Array | null): void {
    ...
    this.addNode(
      new BytesNode(
        ...
        IonTypes.BLOB,
        this.encodeAnnotations(this._annotations),
        value
      )
    );
  }
  ...
  writeDecimal(value: Decimal | null): void {
    ...
    let writeCoefficient =
      isNegative || JSBI.notEqual(coefficient, JsbiSupport.ZERO);
    let coefficientBytes: Uint8Array = writeCoefficient
      ? JsbiSerde.toSignedIntBytes(coefficient, isNegative)
      : EMPTY_UINT8ARRAY;

    let bufLen =
      LowLevelBinaryWriter.getVariableLengthSignedIntSize(exponent) +
      (writeCoefficient ? coefficientBytes.length : 0);
    let writer: LowLevelBinaryWriter = new LowLevelBinaryWriter(
      new Writeable(bufLen)
    );
  ...
  writeSymbol(value: string | null): void {
    ...
    if (value === null) {
      ...
      let writer: LowLevelBinaryWriter = new LowLevelBinaryWriter(
        new Writeable(LowLevelBinaryWriter.getUnsignedIntSize(symbolId))
      );
  ...
  private checkWriteValue(): void {
    ...
    if (this.state === States.STRUCT_FIELD) {
      throw new Error(
        "Expected a struct field name instead of a value, call writeFieldName(string) with the desired name before calling stepIn(IonType) or writeIonType()"
      );
    }
  }
  ...
  private writeSymbolTable() {
    let hasImports: boolean =
      this.symbolTable.import.symbolTable.name != "$ion";
...
export abstract class AbstractNode implements Node {
  ...
  getAnnotatedContainerLength(): number {
    ...
    let annotationsLengthLength = LowLevelBinaryWriter.getVariableLengthUnsignedIntSize(
      annotationsLength
    );
  ...
  getAnnotationsLength(): number {
    if (this.hasAnnotations()) {
      ...
      let containedValueLength = this.getContainedValueLength();
      let containedValueLengthLength = AbstractNode.getLengthLength(
  ...
  writeAnnotations(): void {
    ...
    this.writeTypeDescriptorAndLength(
      TypeCodes.ANNOTATION,
      false,
      annotatedContainerLength
    );
...
class IntNode extends LeafNode {
  ...
  constructor(
    ...
    if (JSBI.GT(this.value, 0)) {
      ...
      let writer: LowLevelBinaryWriter = new LowLevelBinaryWriter(
        new Writeable(LowLevelBinaryWriter.getUnsignedIntSize(magnitude))
      );
