...
//     
//    This decimal is limited to 15 digits of precision but has a
//    exponent range that is +/- 15 digits as well
//   
//    It also accepts 'e', 'E', 'f' and 'F' as valid starts fo
...
export class Decimal 
  ...
  isZeroZero() : boolean 
    if (this.isZero()) 
      // TODO - is this right? negative scale is valid decimal place
      if (this._exponent >= -1) 
  ...
  stringValue(): string 
    ...
    if (this.isNull()) { // is nul
      return "null.decimal"
    

    image = v.digits()
    if (s < 0) 
      // negative shift - prefix decimal point this may require leading zero'
      if (image.length < s+1) 
  ...
  static parse(str: string) : Decimal 
    ...
    for (let ii: number = 0; ii < len; ii++) 
      ...
      case 53: case 54: case 55: case 56: case 57:  // '5' - '9
        ...
        break
      case  68: case  69: case  70: // D, E, F  - exponent star
      case 100: case 101: case 102: // d, e, 
        v = temp;  // store this as the valu
        temp = 0;  // not switch to adding up the exponen
        // check for an exponent sig
        ...
        if (!is_digit(c)) 
          ii = len; // we're done - break us out of the for loo
        
        past_decimal = false; // turn off any scale shifting, we're not in the value part any longe
        break
      default
        ii = len; // we're done - break out of the for loo
      
    
    if (v === undefined) {
      // we never saw an exponent character, so our value is still in tem
      v = temp
    
    else 
      // we did see an exponent, so v is loaded and we need to adjust
      // the exponent_shift we built up byte the user exponen
      if (exp_is_negative) 
        exponent -= temp
