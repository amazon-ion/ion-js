...
export class JsbiSerde {
    private static readonly MAX_SERIALIZED_JSBI_SIZE_IN_BYTES = 12;
    ...
    public static toUnsignedIntBytes(value: JSBI): Uint8Array {
        ...
        let sizeInBytes = this._uintSizeInBytes(value);
    ...
    private static _uintSizeInBytes(value: JSBI): number {
        ...
        throw new Error(
            ...
            + value.toString()
            + " requires more than "
            + this.MAX_SERIALIZED_JSBI_SIZE_IN_BYTES
            + " to serialize as an unsigned int."
        );
    }
    ...
    private static calculateSizeThresholds(): Array<JSBI> {
        ...
        for (let m = 1; m <= this.MAX_SERIALIZED_JSBI_SIZE_IN_BYTES; m++) {
            let exponent: JSBI = JSBI.multiply(JSBI.BigInt(m), this.BITS_PER_BYTE);
            let threshold = JSBI.exponentiate(JsbiSupport.TWO, exponent);
            threshold = JSBI.subtract(threshold, JsbiSupport.ONE);
