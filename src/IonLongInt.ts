...
//       additional semantics necessary for long integer
/
// ION.LongInt supports
/
//      numberValue(
//      byteValue() - returns the bytes of the
//      signum() - return -1, 0, or +
//      isZero(
//      isNull(
//      isNegativeZero(
//      toString(
...
export class LongInt 
  ...
  private static _div_d(bytes: number[], digit: number) 
    // destructive in place divide by digi
  ...
  private _d() : void { // forces creation of base 10 strin
    var dec, str, bytes, len, dg, src, dst
    if (this.d === undefined) 
      if (this.isZero()) 
        this.d = LongInt.zero_string
      } else 
        bytes = LongInt._make_copy(this.b); // make a cop
        len = bytes.length
  ...
  private static _add(bytes: number[], v: number) : void 
    ...
    for (dst = l; dst >= 0; ) {  // we do all digit
      dst--
      t = bytes[dst] + v
      bytes[dst] = t & LongInt.byte_mask;  // bottom 8 bits are the new digi
      v = t >> LongInt.byte_shift;             // bits above 8 are carr
  ...
  private _b() { // forces creation of base 256 byte arra
    ...
    if (this.b === undefined) 
      if (this.isZero()) 
        ...
        // we start at 1 because we always want at least 1 byte in the arra
  ...
  numberValue() : number 
    ...
    len = bytes.length
    for (ii=0; ii<len; ii++) 
      n = (n * LongInt.byte_base) + bytes[ii]; // not shift so that floating point will wor
    
    return n * this.s; // apply the sig
  ...
  static parse(str: string) : LongInt 
    ...
    case LongInt.char_minus
      signum = -1
      // fall through to plus, then to defaul
    case LongInt.char_plus
      dec = dec.slice(1)
      // fall throug
    default
      for (ii=0; ii<dec.length; ii++) { // strip leading zero'
        if (dec.charCodeAt(ii) !== LongInt.char_zero) break;  // '0
      
      if (ii < dec.length) { // first trim the leading zero'
        dec = dec.slice(ii)
  ...
  static fromBytes(bytes: number[], signum: number) : LongInt 
    ...
    // input array is in order of high to lo
    for (ii=0; ii < len; ii++) 
      if (bytes[ii] !== 0) break
    
    if (ii >= len) 
      if (signum === 1) signum = 0; // we don't convert a -0, just a +somethin
