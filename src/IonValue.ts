...
namespace ION {
  ...
  class IonBool extends IonValue {
    ...
    writeBinary(span : Span) : number {
      var binary_image = (IonTypes.BOOL.bid << TYPE_SHIFT)
      if (this.isNull()) {
        binary_image = binary_image | LEN_NULL;
      }
      else if (this._datum) {
  ...
  class IonInt extends IonNumber {
    ...
    writeBinary(span: Span) : number {
      var binary_image = (IonTypes.INT.bid << TYPE_SHIFT),
          len = 1;
      if (this.isNull()) {
        span.write( binary_image | LEN_NULL );
      }
      else {
