...
// Ion Value(s).  This is the Ion DOM for Javascrip
// it holds a tree representation of an Ion value as 
// set of javascript objects derived from IonValue usin
...
abstract class IonValue 
  ...
  // Determines whether this in an Ion null-value, e.g., null or null.strin
  isNull() : boolean 
    // we use _dataum undefined to represent any IonValue typ
    // which has not been set or is explicitly set to nul
...
class IonNull extends IonValue 
  ...
  readBinary(span: Span, bid: number, len: number) : number 
    ...
    return len; // additional bytes rea
...
class IonInt extends IonNumber 
  ...
  writeBinary(span: Span) : number 
    ...
    if (this.isNull()) 
      ...
      if (v === 0.0) 
        span.write( binary_image );  // zero is represented by a 0 length in
        ...
        // NOTE: the real limit for Javascript is a
        //       int that's 53 bits long as JS use
        //       64 binary floating point to stor
