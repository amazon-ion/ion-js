...
describe("IonParserTextRaw", () => {
  [
    ...
    },
    {
      ...
      tests: [
        ...
        { ion: "0", expected: BigInt("0") },
        { ion: "1", expected: BigInt("1") },
        { ion: "-1", expected: BigInt("-1") },
      ],
    },
    {
    ...
    },
    {
      ...
      T_id: 9, // T_IDENTIFIER
      tests: [{ ion: "null.symbol", expected: null }, { ion: "hello" }],
    },
    {
      type: IonTypes.STRING,
      T_id: 12, // T_STRING2
      tests: [
    ...
    },
    {
      type: IonTypes.LIST,
      T_id: 18, // T_LIST
      tests: [{ ion: "null.list", expected: null }],
    },
    {
      type: IonTypes.SEXP,
      T_id: 17, // T_SEXP
      tests: [{ ion: "null.sexp", expected: null }],
    },
    {
      type: IonTypes.STRUCT,
      T_id: 19, // T_STRUCT
      tests: [{ ion: "null.struct", expected: null }],
    },
  ].forEach((testSet) => {
      ...
      testSet.tests.forEach((test) => {
        it(test.ion, () => {
          ...
          if (p.isNull()) {
            ...
            switch (testSet.type) {
              case IonTypes.INT:
                assert.equal(p.numberValue(), Number(test.expected));
                break;
              case IonTypes.FLOAT:
                isNaN(test.expected)
                  ? assert.isNaN(p.numberValue())
            ...
            switch (testSet.type) {
              case IonTypes.CLOB:
                assert.throws(() => p.get_value_as_string(testSet.T_id));
                assert.deepEqual(
                  p.get_value_as_uint8array(testSet.T_id),
                  test.expected
                );
                break;
              default:
                assert.equal(
                  p.get_value_as_string(testSet.T_id),
  ...
  it("Reads annotations", () => {
    let p = new ParserTextRaw(
      new StringSpan("z::[1, y::2, x::{a: w::3, b: v::(s::t::u::4)}, r::5]")
    );
    p.next();
    assert.deepEqual(p.annotations(), [new SymbolToken("z")]);
    p.next();
    assert.deepEqual(p.annotations(), []);
    p.next();
    assert.deepEqual(p.annotations(), [new SymbolToken("y")]);
    p.next();
    assert.deepEqual(p.annotations(), [new SymbolToken("x")]);
    p.next();
    assert.deepEqual(p.annotations(), [new SymbolToken("w")]);
    p.next();
    assert.deepEqual(p.annotations(), [new SymbolToken("v")]);
    p.next();
    assert.deepEqual(p.annotations(), [
      new SymbolToken("s"),
      new SymbolToken("t"),
    ...
    p.next();
    assert.deepEqual(p.annotations(), [new SymbolToken("r")]);
