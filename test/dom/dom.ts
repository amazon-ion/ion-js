...
describe("DOM", () => {
  ...
  it("load() Boolean as any", () => {
    ...
    if (b === false) {
      assert.fail(
        null,
        null,
        "Unreachable. b === false is not considered true."
      );
    }
  ...
  it("load() Integer as Value", () => {
    ...
    assert.isTrue(7n === i.bigIntValue()!);
  });
  ...
  it("load() Timestamp as any", () => {
    ...
    assert.equal(new Date("2020-01-16T20:15:54.066Z").getTime(), t.getTime());
  });
  ...
  it("load() Blob as Value", () => {
    ...
    assert.deepEqual(
      encodeUtf8("To infinity... and beyond!"),
      b.uInt8ArrayValue()
    );
  });
  ...
  it("load() Struct with duplicate fields as any", () => {
    let s: any = load("foo::bar::{" + "age: 55, " + "age: 41" + "}")!;
