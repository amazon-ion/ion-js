...
function test(
  ...
  assert.deepEqual(
    ...
    BigInt(expectedCoefficient),
    "_getCoefficient()"
  );
  assert.equal(
    decimal.isNegative(),
    Object.is(Number(expectedCoefficient), -0) ||
      BigInt(expectedCoefficient) < 0n,
    "coefficient sign"
  );

  assert.equal(decimal.getExponent(), expectedExponent, "_getExponent()");
  assert.equal(
    util._sign(decimal.getExponent()),
    util._sign(expectedExponent),
    "exponent sign"
  );

  assert.equal(
    decimal.isNegative(),
    decimalString.trim()[0] === "-",
    "isNegative()"
  );
  ...
  if (expectedNumberValue instanceof Array) {
    if (expectedNumberValue.length !== 2) {
      throw new Error(
        `Expected number value must be size two ${expectedNumberValue}`
      );
    }
    ...
    assert.isAtLeast(
      ...
      `numberValue() not in range ${expectedNumberValue}`
    );
    assert.isAtMost(
      decNumberValue,
    ...
    assert.equal(
      util._sign(decimal.numberValue()),
      util._sign(expectedNumberValue[0]),
      "numberValue sign"
    );
  } else {
    ...
    assert.equal(
      util._sign(decimal.numberValue()),
      util._sign(expectedNumberValue),
      "numberValue sign"
    );
  }
...
function testEquals(
  decimalString1: string,
  decimalString2: string,
  expected: boolean
) {
...
let decimalParsingTests = [
  ...
  {
    inputString: "123456000.",
    coefficient: "123456000",
    exponent: 0,
    numberValue: 123456000,
    text: "123456000",
  },
  {
    inputString: "123456d-6",
    coefficient: "123456",
    exponent: -6,
  ...
  },
  {
    ...
    coefficient: "123",
    exponent: -10,
    numberValue: 1.2300000000000001e-8,
    text: "1.23E-8",
  },
  {
    inputString: "-123d-12",
    coefficient: "-123",
    exponent: -12,
    numberValue: -0.000000000123,
    text: "-1.23E-10",
  },
  {
    inputString: "12300d-2",
    coefficient: "12300",
    exponent: -2,
    numberValue: 123,
    text: "123.00",
  ...
  {
    inputString: "123456.0",
    coefficient: "1234560",
    exponent: -1,
    numberValue: 123456.0,
    text: "123456.0",
  },
  {
    inputString: "1234560d-1",
    coefficient: "1234560",
    exponent: -1,
    numberValue: 123456.0,
    text: "123456.0",
...
let decimalEqualsTests: {
  ...
  {
    input1: "10000000000000000.00000000000000001",
    input2: "10000000000000000.00000000000000001",
    expected: true,
  },
  {
    input1: "10000000000000000.00000000000000001",
    input2: "10000000000000000.00000000000000002",
    expected: false,
  },
  {
    input1: "10000000000000000.00000000000000000",
    input2: "10000000000000000.0000000000000000",
    expected: false,
  },
];

let decimalFromNumberNumberTests: {
  coefficient: number;
  exponent: number;
}[] = [
...
let isNegativeZero = (
  ...
  return (
    (typeof value === "number" && Object.is(-0, value)) ||
    (typeof value === "bigint" && isNegative === true && value === 0n)
  );
};

let toStringWithSign = (
  value: number | bigint,
  isNegative: boolean
): string => {
...
let decimalConstructorTest = (
  coefficient: number | bigint,
  exponent: number,
  ...
  it(testName, () => {
    ...
    assert.deepEqual(BigInt(coefficient), decimalValue.getCoefficient());
...
describe("Decimal", () => {
  describe("Constructor variants", () => {
    ...
    describe("(BigInt, number, boolean)", () => {
      decimalFromNumberNumberTests.forEach(({ coefficient, exponent }) => {
        ...
        let bigIntCoefficient = BigInt(coefficient);
  ...
  describe("equals", () => {
    decimalEqualsTests.forEach(({ input1, input2, expected, skip = false }) => {
      ...
      it(testName, () => testEquals(input1, input2, expected));
    });
  });

  describe("compareTo", () => {
    decimalComparisonTests.forEach(({ input1, input2, expected }) => {
      it(input1 + " compareTo " + input2, () =>
  ...
  it("intValue(5.0)", () =>
    assert.equal(ion.Decimal.parse("5.0")!.intValue(), 5));
  it("intValue(5.0000001)", () =>
    assert.equal(ion.Decimal.parse("5.0000001")!.intValue(), 5));
  it("intValue(5.9999999)", () =>
    assert.equal(ion.Decimal.parse("5.9999999")!.intValue(), 5));
  it("intValue(0)", () => assert.equal(ion.Decimal.parse("0")!.intValue(), 0));
  it("intValue(0.0000001)", () =>
    assert.equal(ion.Decimal.parse("0.0000001")!.intValue(), 0));
  it("intValue(0.9999999)", () =>
    assert.equal(ion.Decimal.parse("0.9999999")!.intValue(), 0));
  it("intValue(-0)", () => {
  ...
  it("intValue(-1.0000001)", () => () =>
    assert.equal(ion.Decimal.parse("-1.0000001")!.intValue(), -1));
});
