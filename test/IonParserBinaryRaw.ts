...
let unsignedIntBytesMatchValue = (
  bytes: number[],
  expected: number,
  readFrom: (
    input: BinarySpan,
    numberOfBytes: number
  ) => any = ParserBinaryRaw._readUnsignedIntAsBigIntFrom
) => {
  let binarySpan = new BinarySpan(new Uint8Array(bytes));
  let actual = readFrom(binarySpan, bytes.length);
  actual = typeof actual === "bigint" ? Number(actual) : actual;
...
describe("Reading unsigned ints", () => {
  ...
  describe("Safe", () => {
    unsignedIntReadingTests.forEach(({ bytes, expected }) => {
      it("Reading " + expected + " from bytes: " + bytes.toString(), () =>
        unsignedIntBytesMatchValue(
          bytes,
          expected,
          ParserBinaryRaw._readUnsignedIntAsNumberFrom
        )
      );
    });
  });
...
let signedIntBytesMatch = function (
  bytes: number[],
  expected: SignAndMagnitudeInt
) {
  let binarySpan = new BinarySpan(new Uint8Array(bytes));
...
let varUnsignedIntBytesMatchValue = function (
  bytes: number[],
  expected: number
) {
  let binarySpan = new BinarySpan(new Uint8Array(bytes));
...
let varUnsignedIntOverflowTests = [
  ...
  {
    bytes: [0x7f, 0x7f, 0x7f, 0x7f, 0x7f, 0xff],
    expected: maxValueForBits(42),
  },
];
...
let serializeFloat = function (
  value: number,
  viewType: Float32ArrayConstructor | Float64ArrayConstructor,
  numberOfBytes: number
) {
...
let floatBytesMatchValue = function (
  bytes: Uint8Array,
  expected: number,
  comparison = (x: number, y: number) => assert.equal(x, y)
) {
  let binarySpan = new BinarySpan(bytes);
  let actual = ParserBinaryRaw._readFloatFrom(
    binarySpan,
    binarySpan.getRemaining()
  );
...
float64TestValues.push(Number.MIN_SAFE_INTEGER, Number.MAX_SAFE_INTEGER);
